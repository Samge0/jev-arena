...
TETRIS_INSTR = (
    ...
    "(3) among ties, keep the resulting stack height low. "
    "Placements that tie on all three are equivalent."
)
...
def tetris_state_v3(env):
    ...
    return "\n".join([
        ...
        f"Lines cleared so far: {env.lines}.",
        ruler,
        *rows,
    ])
...
def compose_tetris(action_ans, noul_ans, legal, desc):
    ...
    options by (clears desc, holes asc, height asc) computed from desc text."""
    ...
    def key(a):
        ...
        holes = int(d.split("total holes ")[1].split(" ")[0].rstrip("+")) if "total holes" in d else 99
        rest = d.split("rests on row ")[1].split(")")[0] if "rests on row " in d else "99"
        return (-clears, holes, int(rest))
